...
*/
function getName(pObj) {
  return `Hello, my name is ${pObj.name}`
}
...
function getCarInfoByIndex(inventory, index) {
  const carInfo = inventory.find((item, selection) => {return selection === index})
...
function getLastCarInfo(inventory) {
  const lastNum = inventory.length - 1
  const lastCar = inventory.find((item, index) => {return index === lastNum})
...
function getCarInfoById(inventory, carID) {
  const myCar = inventory.find((item, index) => {return item.id === carID})
...
function sortCarInventory(inventory) {
 const sortInv = inventory.sort(function(a, b) {
  let aCar = a.car_model.toUpperCase()
  let bCar = b.car_model.toUpperCase() 
